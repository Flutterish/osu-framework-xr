...
public struct TexturedNormal : IVertex<TexturedNormal> {
	...
	public void Link ( Span<int> attribs ) {
		...
		GL.VertexAttribPointer( attribs[1], 3, VertexAttribPointerType.Float, false, Stride, 3 * sizeof( float ) );
		GL.EnableVertexAttribArray( attribs[1] );
		GL.VertexAttribPointer( attribs[2], 2, VertexAttribPointerType.Float, false, Stride, 5 * sizeof( float ) );
