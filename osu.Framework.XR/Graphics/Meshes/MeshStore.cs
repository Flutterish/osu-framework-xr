...
/// </summary>
public class MeshStore : ResourceStore<Mesh> {
	/// <summary>
	...
	/// </summary>
	public async Task<T> GetAsync<T> ( string name ) where T : Mesh {
		return (T)await GetAsync( name );
	}
	...
	/// </summary>
	public async Task<T> GetNewAsync<T> ( string name ) where T : Mesh {
		return (T)await GetNewAsync( name );
	}
